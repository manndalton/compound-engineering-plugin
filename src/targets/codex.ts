...
export async function writeCodexBundle(outputRoot: string, bundle: CodexBundle): Promise<void> {
  ...
  const hasHooks = bundle.hooks != null && Object.keys(bundle.hooks).length > 0

  if (hasHooks) {
    const hooksPath = path.join(codexRoot, "hooks.json")
    const backupPath = await backupFile(hooksPath)
    if (backupPath) {
      console.log(`Backed up existing hooks.json to ${backupPath}`)
    }
    await writeText(hooksPath, JSON.stringify({ hooks: bundle.hooks }, null, 2) + "\n")
  }

  const config = renderCodexConfig({ mcpServers: bundle.mcpServers, hasHooks })
  if (config) {
...
export function renderCodexConfig(options: CodexConfigOptions = {}): string | null {
  const { mcpServers, hasHooks } = options
