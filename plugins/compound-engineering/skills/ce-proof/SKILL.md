...
- `STALE_BASE`, `BASE_TOKEN_REQUIRED`, `MISSING_BASE`, `INVALID_BASE_TOKEN`, `ANCHOR_NOT_FOUND`, `ANCHOR_AMBIGUOUS` — pre-commit. Safe to retry after re-reading `/state` and refreshing the payload (tighter anchor, fresh token).
