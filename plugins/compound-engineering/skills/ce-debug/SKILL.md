...
3. **Run `/ce-commit-push-pr`.** When the entry came from an issue tracker, the PR description should include the appropriate auto-close syntax for that tracker (e.g., `Fixes #N` for GitHub, `Closes ABC-123` for Linear, Smart Commit syntax for Jira) so the diagnosis and fix flow back to the issue and it closes on merge. Surface the resulting PR URL.
