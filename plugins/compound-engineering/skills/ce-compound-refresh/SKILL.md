...
- Ask questions **one at a time** — use the platform's blocking question tool (`AskUserQuestion` in Claude Code, `request_user_input` in Codex, `ask_user` in Gemini). If the tool is unavailable or errors, present numbered options in plain text and wait for the user's reply — never silently skip the question
...
Always present choices using the platform's blocking question tool (`AskUserQuestion` in Claude Code, `request_user_input` in Codex, `ask_user` in Gemini). If the tool is unavailable or errors, present numbered options in plain text and wait for the user's reply — never silently skip the question.
